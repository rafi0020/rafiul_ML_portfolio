...
const canonicalUrl = route => `https://www.rafiulislam.me${route === '/' ? '/' : `${route}/`}`;
let assets = 0;
for (const route of routes) {
  ...
  assert(html.includes(`href="${canonicalUrl(route)}"`), `${route}: incorrect canonical`);
  ...
  for (const [,value] of html.matchAll(/content="(https:\/\/www\.rafiulislam\.me\/assets\/[^\"]+)"/g)) assert(fs.existsSync(path.join('dist',new URL(value).pathname)), `${route}: missing social asset`);
...
for (const route of routes) assert(sitemap.includes(`${canonicalUrl(route)}</loc>`), `Sitemap missing ${route}`);
